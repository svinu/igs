...
public class AdminClaim {
	...
	public static void claimCreation(UserRole user) {
		...
		/*
		 * List<PolicyEntity> list; try { list = service.getPolicies(user);
		 * System.out.println("********** Policy List **********"); for
		 * (PolicyEntity entity : list) {
		 ...
		 * entity.getAccountNumber()); System.out.println(" ");
		 * 
		...
		if (validatePolicyNumber) {
			...
			do {
				...
				if (validateClaimReason == false) {
					System.err
							.println("The reason should always starts with uppercase");
					doClaimReason = false;
				} else {
					doClaimReason = true;
				}
			...
			if (validateClaimReason) {
				...
				do {
					...
					if (validateAccidentLocationStreet == false) {
						System.err
								.println("The Street name should always starts with uppercase");
				...
				if (validateAccidentLocationStreet) {
					...
					do {
						...
						if (validateAccidentCity == false) {
							System.err
									.println("The City name should always starts with uppercase");
					...
					if (validateAccidentCity) {
						...
						do {
							...
							if (validateAccidentState == false) {
								System.err
										.println("The State should always starts with uppercase");
								...
								do {
									...
									try {
										...
										default:
											...
											System.err
													.println("Select any options from 1 to 3");
											break;
										}
									} catch (InputMismatchException e) {
										System.err
								...
								try {
									...
									do {
										...
										if (claimDetails.equals("Yes")
												|| claimDetails.equals("yes")
												|| claimDetails.equals("Y")
												|| claimDetails.equals("y")) {
											doYesNo = true;
											ClaimQuestions.claimQuestion(user);
										} else if (claimDetails.equals("No")
												|| claimDetails.equals("no")
												|| claimDetails.equals("n")
												|| claimDetails.equals("N")) {
